...
class TestFunction {
	...
	public int decompress(byte[] bytes, byte[] input, int uncompressed_size) {
		...
		if(uncompressed_size == 16725) {
			__input = new Pointer(1000000).WriteBytes(bytes);
			__param_2 = new Pointer(1000000);
			int __result = DecompressJava2(__input, __param_2, uncompressed_size);
...
/**			[[[ Follow though ]]]						   */
			...
			if(!(EDX <= EAX)) {
				...
				if(ESI < ECX) { // '>=' jump
					...
					for(;;) { // LAB_00703b90:
						EAX = param_2.Int(EDX + ECX*0x1);				Log("MOV EAX,dword ptr [EDX + ECX*0x1]");
						param_2.WriteInt(EAX, ECX);					Log("MOV dword ptr [ECX],EAX");
						EAX = param_2.Int(EDX + ECX*0x1 + 0x4);		Log("MOV EAX,dword ptr [EDX + ECX*0x1 + 0x4]");
						...
						if(ECX < EBX) continue; // TODO - Check if this is correct
...
//			RET                                     // 00703a03
	...
	private Address CALL(Instruction curr, String[] split) {
		...
		if(offset == 0x00d20240) { // (C++) memmove
			...
			//System.out.println("memmove(" + idst + ", " + isrc + ", " + num + ")");
